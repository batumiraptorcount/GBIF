...
### load data #####
## modify this
#count_data<-read.csv("BRC-Bitbucket/CountData.csv", header=TRUE, encoding="UTF-8") 
lastid<-max(read.csv("final/PROCESSED-allyears_withFilter.csv", header=TRUE, encoding="UTF-8")$id)
count_data<-read.csv("in/dataconverter_2018.csv", header=TRUE, encoding="UTF-8")
#dc_specs<-read.csv("BRC-Bitbucket/dc_specs.csv", header=TRUE, encoding="UTF-8") 
dc_specs<-read.csv("final/dc_specs.csv", header=TRUE, encoding="UTF-8")
#dc_specs_nonsoaring<-read.csv("BRC-Bitbucket/dc_specs_nonsoaring.csv", header=TRUE, encoding="UTF-8") 
dc_specs_nonsoaring<-read.csv("final/dc_specs_nonsoaring.csv", header=TRUE, encoding="UTF-8")
...
count_data$id<-seq.int(from= lastid, to= lastid+nrow(count_data)-1)
...
for (y in min(count_data$year):max(count_data$year)){
	...
	if(unique(station1$year)>2009){
		...
		HB_BK_age_frame<-! which(speciesname1 == "HB_NONJUV" | speciesname1 == "HB_JUV" | speciesname1 == "BK_AD" | speciesname1 == "BK_NONJUV")
...
#	write.csv(new_count, paste0("final/1-new_count_DC_",unique(year(new_count$date)),".csv"), fileEncoding ="UTF-8", row.names=FALSE)
	write.csv(dcdel_tab, paste0("out/dcdel/tab_", unique(year(new_count$date)),".csv"), fileEncoding ="UTF-8", row.names=FALSE)
...
### write results
#modify path
#write.csv(new_count_all_data, paste0("BRC-Bitbucket/PROCESSED-allyears.csv"), fileEncoding ="UTF-8", row.names=FALSE) 
write.csv(new_count_all_data, paste0("final/PROCESSED-2018.csv"), fileEncoding ="UTF-8", row.names=FALSE) 
